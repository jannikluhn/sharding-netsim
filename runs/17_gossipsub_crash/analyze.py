...
def plot_hists(results):
    ...
    for i, results in enumerate(all_results):
        ...
        crash_probability = float(results.itervars["c"])

        ax.plot(
            bins[1:],
            cum_hist,
            color=colormap(i),
            label=f"{crash_probability * 100:.0f}%",
            **plot_kwargs,
        )
    ...
    ax.set_xlim(0, 16)
    ax.legend(title="Crash probability")
...
def check_message_loss(all_results):
    for i, results in enumerate(all_results):
        ...
        loss = calc_loss(gossip_emitted, gossip_received, node_count)
        if loss > 0:
            print(f"warning: non-zero loss ({loss:.2f}) in run {i}")
